...
root.iconbitmap('"BMI_Calculator\codey.ico"')
...
def get_bmi():
    # calculate BMI
    # (weight_pounds/height_inches^2)
    our_height = (int(h_entry.get()) * int(h_entry.get()))/10000
...
h_entry = customtkinter.CTkEntry(master=root,
                                 placeholder_text="Height In Centimeter",
                                 width=200,
                                 height=30,
                                 border_width=1,
                                 corner_radius=10)
